...
if(isset($_POST['x'])) {
    ...
    $insert_pic_query = mysqli_query(
        $con,
        "UPDATE users SET profile_pic='$result_path' WHERE username='$userLoggedIn'"
    );
    header("Location: " . $userLoggedIn);
}// post x
?>
<div id="Overlay"
     style=" width:100%; height:100%; border:0 #990000 solid; position:absolute; top:0; left:0; z-index:2000; display:none;"></div>
<div class="main_column column">
    ...
    if($imgSrc) { //if an image has been uploaded display cropping area
        ...
        <div id="CroppingContainer"
             style="width:800px; max-height:600px; background-color:#FFF; margin-left: -200px; position:relative; overflow:hidden; border:2px #666 solid; z-index:2001; padding-bottom:0;">

            <div id="CroppingArea"
                 style="width:500px; max-height:400px; position:relative; overflow:hidden; margin:40px 0 40px 40px; border:2px #666 solid; float:left;">
                <img src="<?= $imgSrc ?>" id="jcrop_target"
                     style="border:0 #990000 solid; position:relative; margin:0 0 0 0; padding:0; "/>
            </div>

            <div id="InfoArea"
                 style="width:180px; height:150px; position:relative; overflow:hidden; margin:40px 0 0 40px; border:0 #666 solid; float:left;">
